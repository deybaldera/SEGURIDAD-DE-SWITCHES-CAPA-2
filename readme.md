...
/significa que el puerto soloa aceptara hasta 3 direcciones MAC diferentes en este puerto/

Switch(config-if)# switchport port-security mac-address sticky
/Aprende automáticamente las MAC conectadas y las guarda como permitidas/

S1(config)# interface fa0/1 S1(config-if)# switchport port-security aging time 10 
